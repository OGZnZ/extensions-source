package eu.kanade.tachiyomi.multisrc.astrohwago
...
abstract class AstroHwago : KeiSource() {
    ...
    private fun parseMangaList(document: Document, page: Int): MangasPage {
        val mangas = document.select("a[href^=\"/comic/\"]:has(img:not([src*=\"flagcdn\"]))").mapNotNull { element ->
            ...
            SManga.create().apply {
                ...
                url = element.attr("abs:href").toHttpUrl().pathSegments.lastOrNull()?.takeIf { it.isNotBlank() }
                    ?: element.attr("abs:href").substringAfterLast("/").substringBefore("?").substringBefore("#")
            }
        }.distinctBy { it.url }
    ...
    override fun getMangaUrl(manga: SManga): String {
        val slug = manga.url.substringBefore("?").substringBefore("#").trim().trimEnd('/').substringAfterLast("/")
        return "$baseUrl/comic/$slug"
    }

    private fun mangaPath(url: String): String {
        val slug = url.substringBefore("?").substringBefore("#").trim().trimEnd('/').substringAfterLast("/").trim()
        return "/comic/$slug"
    }
    ...
    override suspend fun fetchMangaUpdate(
        ...
        val document = client.get(baseUrl + mangaPath(manga.url)).asJsoup()
